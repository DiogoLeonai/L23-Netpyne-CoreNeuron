...
namespace coreneuron {
    ...
    static_assert(std::is_trivially_copy_constructible_v<K_T_Store>);
    static_assert(std::is_trivially_move_constructible_v<K_T_Store>);
    static_assert(std::is_trivially_copy_assignable_v<K_T_Store>);
    static_assert(std::is_trivially_move_assignable_v<K_T_Store>);
    static_assert(std::is_trivially_destructible_v<K_T_Store>);
    static K_T_Store K_T_global;
    ...
    static void nrn_private_constructor_K_T(NrnThread* nt, Memb_list* ml, int type) {
        assert(!ml->instance);
        assert(!ml->global_variables);
        assert(ml->global_variables_size == 0);
        auto* const inst = new K_T_Instance{};
        assert(inst->global == &K_T_global);
        ml->instance = inst;
        ml->global_variables = inst->global;
        ml->global_variables_size = sizeof(K_T_Store);
    }

    static inline void copy_instance_to_device(NrnThread* nt, Memb_list* ml, K_T_Instance const* inst);
    static inline void delete_instance_from_device(K_T_Instance* inst);
    // Deallocate the instance structure
    static void nrn_private_destructor_K_T(NrnThread* nt, Memb_list* ml, int type) {
        auto* const inst = static_cast<K_T_Instance*>(ml->instance);
        assert(inst);
        assert(inst->global);
        assert(inst->global == &K_T_global);
        assert(inst->global == ml->global_variables);
        assert(ml->global_variables_size == sizeof(K_T_Store));
    ...
    static void nrn_alloc_K_T(double* data, Datum* indexes, int type) {
        // do nothing
    }


    void nrn_constructor_K_T(NrnThread* nt, Memb_list* ml, int type) {
    ...
    void nrn_destructor_K_T(NrnThread* nt, Memb_list* ml, int type) {
        #ifndef CORENEURON_BUILD
        int nodecount = ml->nodecount;
        int pnodecount = ml->_nodecount_padded;
        const int* node_index = ml->nodeindices;
    ...
    inline static int rates_K_T(int id, int pnodecount, K_T_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v);


    inline int rates_K_T(int id, int pnodecount, K_T_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        int ret_rates = 0;
        double qt;
        qt = pow(2.3, ((34.0 - 21.0) / 10.0));
        v = v + 10.0;
        inst->mInf[id] = 1.0 / (1.0 + exp( -v / 19.0));
    ...
    /** initialize channel */
    void nrn_init_K_T(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            ...
            if (nt->compute_gpu) {
                nrn_pragma_acc(update device (K_T_global))
                nrn_pragma_omp(target update to(K_T_global))
            }
    ...
    inline double nrn_current_K_T(int id, int pnodecount, K_T_Instance* inst, double* data, const Datum* indexes, ThreadDatum* thread, NrnThread* nt, double v) {
        double current = 0.0;
        inst->g[id] = inst->gbar[id] * (pow(inst->m[id], 4.0)) * inst->h[id];
    ...
    /** update current */
    void nrn_cur_K_T(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
            ...
            for (int id = 0; id < nodecount; id++) {
                ...
                #endif
                inst->ek[id] = inst->ion_ek[indexes[0*pnodecount + id]];
                double g = nrn_current_K_T(id, pnodecount, inst, data, indexes, thread, nt, v+0.001);
                double dik = inst->ik[id];
                double rhs = nrn_current_K_T(id, pnodecount, inst, data, indexes, thread, nt, v);
                g = (g-rhs)/0.001;
    ...
    /** update state */
    void nrn_state_K_T(NrnThread* nt, Memb_list* ml, int type) {
        nrn_pragma_acc(data present(nt, ml) if(nt->compute_gpu))
        {
            int nodecount = ml->nodecount;
            int pnodecount = ml->_nodecount_padded;
    ...
    void _K_T_reg() {
        ...
        int mech_type = nrn_get_mechtype("K_T");
        K_T_global.mech_type = mech_type;
        if (mech_type == -1) {
            return;
        }

        _nrn_layout_reg(mech_type, 0);
        register_mech(mechanism_info, nrn_alloc_K_T, nrn_cur_K_T, nullptr, nrn_state_K_T, nrn_init_K_T, nrn_private_constructor_K_T, nrn_private_destructor_K_T, first_pointer_var_index(), 1);
        K_T_global.k_type = nrn_get_mechtype("k_ion");
