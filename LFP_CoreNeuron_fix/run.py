...
def preRun():
    ...
    if ((sim.cfg.recordLFP or sim.cfg.saveIMembrane) and not sim.cfg.coreneuron):
...
def runSim(skipPreRun=False):
    ...
    if sim.cfg.coreneuron == True:
        ...
        if sim.rank == 0:
            print(
                '\nRunning simulation using CoreNEURON '
                'for %s ms...' % sim.cfg.duration
            )

        from neuron import coreneuron

        # ==========================================================
        # Native CoreNEURON LFP
        # ==========================================================
        ...
        if sim.cfg.recordLFP:
            ...
            if not hasattr(
                ...
                raise RuntimeError(
                    '[CoreLFP] LFP was requested with CoreNEURON, '
                    'but setupRecordLFP() did not create '
                    '_coreLFP_sim_config.'
                )
            ...
            if sim.rank == 0:
                ...
                print(
                    '[CoreLFP] CoreNEURON sim_config:',
                    coreneuron.sim_config,
                    flush=True
                )

        # ==========================================================
        # Normal CoreNEURON configuration
        # ==========================================================
