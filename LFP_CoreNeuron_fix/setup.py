...
def setupRecordLFP():
    ...
    Traditional NEURON:
        NetPyNE PtrVector -> i_membrane_ -> calculateLFP()

    CoreNEURON:
        NetPyNE computes electrode transfer resistances,
        then registers them as native CoreNEURON LFP factors.
    """
    ...
    if sim.cfg.coreneuron and sim.cfg.recordLFP:
        lfp_record_step = float(
            getattr(
                sim.cfg,
                'recordLFPStep',
                sim.cfg.recordStep
        ...
        lfp_record_step = float(sim.cfg.recordStep)
    ...
    # --------------------------------------------------------------
    # Optional per-cell LFP
    # --------------------------------------------------------------

    if sim.cfg.saveLFPCells:

        if sim.cfg.saveLFPCells is True:
            cellsRecordLFP = utils.getCellsList(['all'])

        elif isinstance(
            sim.cfg.saveLFPCells,
            list
        ):
            cellsRecordLFP = utils.getCellsList(
                sim.cfg.saveLFPCells
            )

        for c in cellsRecordLFP:

            sim.simData['LFPCells'][c.gid] = (
                np.zeros(
                    (
                        saveSteps,
                        nsites
    ...
    # --------------------------------------------------------------
    # Optional population LFP
    # --------------------------------------------------------------

    if sim.cfg.saveLFPPops:

        if sim.cfg.saveLFPPops is True:
            ...
            popsRecordLFP = [
                p
                for p in sim.cfg.saveLFPPops
                if p in sim.net.pops
            ]
    ...
    # ==============================================================
    # Geometry required by both NEURON and CoreNEURON LFP
    # ==============================================================
    ...
    sim.net.recXElectrode = (
        RecXElectrode.fromConfig(
            sim.cfg
        )
    )

    # ==============================================================
    # Optional iMembrane storage
    #
    # IMPORTANT:
    # Only this small allocation block belongs inside
    # `if sim.cfg.saveIMembrane`.
    # ==============================================================

    if sim.cfg.saveIMembrane:

        if sim.cfg.saveIMembrane is True:

            cellsRecordIMembrane = (
                utils.getCellsList(['all'])
            )

        elif isinstance(
            sim.cfg.saveIMembrane,
            list
        ):
    ...
    # ==============================================================
    # Everything below MUST be outside `if saveIMembrane`
    # ==============================================================
    ...
    if sim.cfg.createNEURONObj:
        ...
        # ==========================================================
        # PASS 1:
        # Calculate transfer resistance for EVERY local cell.
        #
        # NetPyNE stores:
        #
        # transferResistances[gid]
        #
        # and CoreNEURON needs all of these before mapping
        # registration begins.
        # ==========================================================
        ...
        # ==========================================================
        # Validate the transfer-resistance dictionary
        # ==========================================================
        ...
        if sim.cfg.recordLFP:
            ...
            missing_gids = [
                ...
                if gid not in available_gids
            ]

            # print(
            #     f'[LFP setup rank={sim.rank}] '
            #     f'local cells={len(local_gids)}, '
            #     f'transfer matrices={len(available_gids)}, '
            #     f'missing={len(missing_gids)}',
            #     flush=True
            # )
            ...
            if missing_gids:
                ...
                raise RuntimeError(
                    f'[LFP setup rank={sim.rank}] '
                    f'Missing transfer matrices for GIDs: '
                    f'{missing_gids[:20]}'
                )

        # ==========================================================
        # CoreNEURON-native LFP
        # ==========================================================

        if (
            sim.cfg.coreneuron
            and sim.cfg.recordLFP
        ):
            ...
            for i, cell in enumerate(sim.net.compartCells):

                gid = int(cell.gid)

                tr = np.asarray(
                    sim.net.recXElectrode.transferResistances[gid]
                )

                total_nonzero += np.count_nonzero(tr)
                total_values += tr.size
                ...
                global_absmax = max(
                    global_absmax,
                    float(np.max(np.abs(tr)))
                )

            # print(
            #     f'[LFP transfer rank={sim.rank}] '
            #     f'ALL CELLS: min={global_min}, '
            #     f'max={global_max}, '
            #     f'absmax={global_absmax}, '
            #     f'nonzero={total_nonzero}/{total_values}',
            #     flush=True
            # )
            ...
            if sim.rank == 0:
                ...
                print(
                    '[CoreLFP] All transfer matrices calculated. '
                    'Registering CoreNEURON LFP...',
                    flush=True
                )

            from .coreneuron_lfp import (
                setup_core_lfp
            )
            ...
            # ------------------------------------------------------
            # Verify that the helper really completed.
            # ------------------------------------------------------
            ...
            if not hasattr(
                ...
                raise RuntimeError(
                    '[CoreLFP] setup_core_lfp() returned '
                    'without creating '
                    'sim._coreLFP_sim_config.'
                )

            # print(
            #     f'[CoreLFP rank={sim.rank}] '
            #     f'setup complete: '
            #     f'{sim._coreLFP_sim_config}',
            #     flush=True
            # )
            ...
            # IMPORTANT:
            # do not create NetPyNE PtrVectors below.
            return

        # ==========================================================
        # Traditional NEURON implementation
        # ==========================================================

        for cell in sim.net.compartCells:

            nseg = (
                cell.getNumberOfSegments()
            )

            cell.imembPtr = h.PtrVector(
                nseg
            )
